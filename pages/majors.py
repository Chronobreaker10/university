...
from core.schemas import MajorCreate, MajorRead, UserRead, MajorResponse
...
def get_majors(request: Request, data: Annotated[MajorResponse, Depends(get_all_majors)],
    ...
    return templates.TemplateResponse("majors/index.html",
                                      {"request": request, "response": data, "title": "Специальности", "message": message,
...
async def post_major(request: Request, major: Annotated[MajorCreate, Form()],
    ...
    request.session["flashed_message"] = {
        "type": "success",
        "text": f"Специальность {major_id} успешно добавлена!"
    }
    request.session.pop("flashed_data")
