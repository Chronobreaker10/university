...
from core.schemas import UserCreate, UserRead, UserAuth, Token
...
@router.post("/register", summary="Зарегистрироваться", response_model=UserRead, status_code=status.HTTP_201_CREATED)
async def register(session: Annotated[AsyncSession, Depends(db_helper.get_session())], user: UserCreate):
    return await service.register_user(session, user)


@router.post("/login", summary="Войти", response_model=Token)
async def login(session: Annotated[AsyncSession, Depends(db_helper.get_session())], response: Response, credentials: UserAuth):
