...
async def get_all_majors(session: Annotated[AsyncSession, Depends(get_session())],
...
async def get_one_major_by_id(session: Annotated[AsyncSession, Depends(get_session())],
...
async def add_major(session: Annotated[AsyncSession, Depends(get_session())], major: MajorCreate):
...
async def update_major(session: Annotated[AsyncSession, Depends(get_session())],
                       major_id: Annotated[int, Path(ge=1, le=1000, title="ID", description="ID")],
...
async def update_major_by_name(session: Annotated[AsyncSession, Depends(get_session())],
...
async def delete_major(session: Annotated[AsyncSession, Depends(get_session())],
                       major_id: Annotated[int, Path(ge=1, le=1000, title="ID", description="ID")]):
